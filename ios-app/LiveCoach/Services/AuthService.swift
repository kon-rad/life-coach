...
@Observable final class AuthService: NSObject {
    ...
    func signInWithGoogle() async throws {
        isLoading = true
        defer { isLoading = false }
        // GoogleSignIn needs a client ID before signIn(withPresenting:) is called,
        // otherwise it throws immediately. Read it from the Firebase options
        // (populated by GoogleService-Info.plist) so it's not duplicated.
